...
export const userSchemas = {
  register: Joi.object({
    username: Joi.string().pattern(/^[a-zA-Z0-9_]+$/).min(3).max(30).required().messages({
      ...
      "string.pattern.base": "Username hanya boleh berisi huruf, angka, dan underscore (_).",
      "any.required": "Username wajib diisi."
    }),
    name: Joi.string().pattern(/^[a-zA-Z\s]+$/).min(3).max(50).required().messages({
    ...
    .items(
        ...
        "string.guid": "ID Role '{#value}' tidak valid. Pastikan formatnya benar.",
  ...
  update: Joi.object({
    ...
    name: Joi.string().pattern(/^[a-zA-Z0-9\s]+$/).min(3).max(50).optional().messages({
      "string.pattern.base": "Nama lengkap hanya boleh berisi huruf, angka, dan spasi."
    }),
